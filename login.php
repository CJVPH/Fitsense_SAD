<?php
/**
 * FitSense — Member Login Page
 * Requirements: 3.1–3.3, 3.7, 3.9
 */
session_start();
...
// Redirect already-authenticated members
if ($auth->isSessionValid() && isset($_SESSION['role']) && $_SESSION['role'] === 'member') {
...
// Check maintenance mode — show notice but still allow login attempt
$pdo         = Database::getConnection();
$maintenance = getSystemSetting('maintenance_mode', $pdo, 'false') === 'true';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Member Login — FitSense</title>
...
<body class="bg-black min-h-screen flex items-center justify-center px-4 py-8" style="min-width:375px">
    ...
    <div class="w-full max-w-sm">
        ...
        <div class="bg-zinc-900 border border-zinc-700 rounded-2xl p-6 shadow-xl">
            ...
            <h1 class="text-white text-2xl font-bold mb-6 text-center">Member Login</h1>
            ...
            <?php if ($maintenance): ?>
            <div class="border border-yellow-400 text-yellow-300 bg-yellow-950 rounded-lg p-3 mb-4 text-sm text-center" role="alert">
                🔧 FitSense is currently under maintenance. Members cannot log in right now.
            </div>
            <?php else: ?>
        ...
        form.addEventListener('submit', async function (e) {
            ...
            try {
                const res = await fetch('api/auth.php', {
                    ...
                    body:    JSON.stringify({ action: 'login', username, password, csrf_token })
                });
